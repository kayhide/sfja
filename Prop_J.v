...
[[
     foo'_ind :
        forall (X : Type) (P : foo' X -> Prop),
              (forall (l : list X) (f : foo' X),
                    P f____________________ ->
                    P (C1 X l f)___________   ) ->
             P (C2 X)___________________________________ ->
             forall f1 : foo' X, P f1___________________
]]
*)
Check foo'_ind.
(** [] *)
...
(** [ev_MyProp] の 形式的な証明に対応する非形式的な証明を書きなさい。
    ...
    証明: 
      MyProp 4 の場合
        ev_SS により ev 2 であれば ev 4 である。
        ev_SS により ev 0 であれば ev 2 である。
        ev_0 により ev 0 である。
        よって ev 4 である。

      MyProp (4 + x) の場合
        ev_SS により ev (2 + x) であれば ev (4 + x) である。
        ev_SS により ev x であれば ev (2 + x) である。
        帰納法の仮定より ev x である。
        よって ev (4 + x) である。

      MyProp x の場合
        ev の定義により ev (2 + x) は ev_0 あるいは ev_SS のいずれかである。
        ev_0 の場合
          (2 + x) = 0 に矛盾する。
        ev_SS の場合
          ev_SS の定義により ev (2 + x) は ev x である。
          帰納法の仮定より ev (2 + x) である。
          よって ev x である。

      以上により、全ての自然数 [n] に対して [MyProp n] ならば [ev n] である。
[] *)
